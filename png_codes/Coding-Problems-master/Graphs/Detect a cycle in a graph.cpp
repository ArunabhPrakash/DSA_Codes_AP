...
class Graph{//detection in a directed graph
int V;
list<int> *adj;
...
bool isCyclicHelper(int s,bool *visited,bool *rStack){
        ...
        for(auto it=adj[s].begin();it!=adj[s].end();it++){
                if(!visited[*it]&&isCyclicHelper(*it,visited,rStack)){//dfs calling if not visited to check
                    return true;
                }
                else if(rStack[*it]){//if visited and also present in stack
                    return true;
                }
            }
